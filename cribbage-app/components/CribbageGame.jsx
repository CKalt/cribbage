                          ...
                          <div className="relative w-12 h-16">
                            {pileCount === 0 ? (
                              ...
                              <>
                                ...
                                {pileCount >= 4 && <div className={`absolute top-3 left-1.5 ${cardBack.bg} border-2 ${cardBack.border} rounded w-12 h-16 shadow-lg overflow-hidden flex items-center justify-center ${cardBack.iconColor}`}><div className="absolute inset-0" style={{ background: cardBack.pattern }} /><span className={`relative select-none ${cardBack.centerIcon.length > 1 ? 'text-2xl' : 'text-base'}`}>{cardBack.centerIcon}</span><span className="absolute top-0.5 left-0.5 select-none" style={{ fontSize: cardBack.centerIcon.length > 1 ? '10px' : '6px', lineHeight: 1 }}>{cardBack.centerIcon}</span><span className="absolute bottom-0.5 right-0.5 select-none" style={{ fontSize: cardBack.centerIcon.length > 1 ? '10px' : '6px', lineHeight: 1, transform: 'rotate(180deg)' }}>{cardBack.centerIcon}</span></div>}
                              </>
                            )}
                          </div>
