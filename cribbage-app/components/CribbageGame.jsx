                          ...
                          <div ref={deckPileRef} className="relative" style={{ width: '40px', height: '56px' }}>
                            {Array.from({ length: Math.min(3, remaining) }).map((_, i) => (
                              ...
                              >
                                ...
                                {cardBack.type === 'fullcard' ? (cardBack.sceneSvg ? <div className="absolute inset-0" dangerouslySetInnerHTML={{ __html: cardBack.sceneSvg }} /> : <span className="relative select-none" style={{ fontSize: '40px', lineHeight: 1, transform: 'scaleY(1.4)' }}>{cardBack.centerIcon}</span>) : <span className={`relative ${cardBack.iconColor} select-none ${cardBack.centerIcon.length > 1 ? 'text-lg' : 'text-sm'}`}>{cardBack.centerIcon}</span>}
                              </div>
                            ))}
                          </div>
