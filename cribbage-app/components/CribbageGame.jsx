                        ...
                        <div ref={cribPileRef} className="flex flex-col items-center">
                          <div className="relative w-12 h-16">
                            {pileCount === 0 ? (
                              ...
                              <>
                                {pileCount >= 1 && <div className={`absolute top-0 left-0 ${cardBack.bg} border-2 ${cardBack.border} rounded w-12 h-16 shadow-md overflow-hidden`}>{cardBack.sceneImage ? <img src={cardBack.sceneImage} alt="" className="absolute inset-0 w-full h-full object-cover rounded" draggable={false} /> : <div className="absolute inset-0" style={{ background: cardBack.pattern }} />}</div>}
                                {pileCount >= 2 && <div className={`absolute top-1 left-0.5 ${cardBack.bg} border-2 ${cardBack.border} rounded w-12 h-16 shadow-md overflow-hidden opacity-90`}>{cardBack.sceneImage ? <img src={cardBack.sceneImage} alt="" className="absolute inset-0 w-full h-full object-cover rounded" draggable={false} /> : <div className="absolute inset-0" style={{ background: cardBack.pattern }} />}</div>}
                                {pileCount >= 3 && <div className={`absolute top-2 left-1 ${cardBack.bg} border-2 ${cardBack.border} rounded w-12 h-16 shadow-lg overflow-hidden opacity-80`}>{cardBack.sceneImage ? <img src={cardBack.sceneImage} alt="" className="absolute inset-0 w-full h-full object-cover rounded" draggable={false} /> : <div className="absolute inset-0" style={{ background: cardBack.pattern }} />}</div>}
